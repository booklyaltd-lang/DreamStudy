...
- `https://bizdevblog.ru/blog/your-article-slug` - должна открываться страница статьи
- Обновление страницы не должно приводить к 404 ошибке
- Кнопки назад/вперед в браузере должны работать корректно
- При шаринге в социальных сетях ссылка должна вести на статью, а не на главную

## Особенности работы роутинга

Приложение использует HTML5 History API для навигации:
- При клике на ссылки внутри приложения URL меняется без перезагрузки страницы
- При прямом переходе по URL приложение загружает нужную страницу
- Файлы `_redirects`, `vercel.json` и `.htaccess` перенаправляют все запросы на `index.html`, где React берет управление роутингом
