...
2. **Создание платежа**: Edge Function `create-payment` создает запись в БД со статусом `pending`
3. **Открытие виджета**: На странице открывается виджет CloudPayments
4. **Оплата**: Пользователь вводит данные карты и оплачивает
5. **Успех виджета**: Виджет закрывается и показывает сообщение об успешной оплате
6. **Webhook от CloudPayments**: CloudPayments АВТОМАТИЧЕСКИ отправляет уведомление на webhook (5-30 секунд)
7. **Обработка**: Webhook обновляет статус платежа на `succeeded` и создает подписку/покупку курса
8. **Доступ**: Пользователь получает доступ к контенту (можно обновить страницу)

**ВАЖНО**: Пользователь НЕ ДОЛЖЕН ничего подтверждать вручную! Все происходит автоматически через webhook.

## Обработка платежей

Система использует webhook CloudPayments для автоматической обработки:

### Webhook (единственный путь обработки)
- CloudPayments автоматически отправляет уведомление после обработки платежа
- Обработка занимает 5-30 секунд
- **Требует обязательной настройки в личном кабинете CloudPayments (см. Шаг 4)**
- После получения webhook статус платежа меняется с `pending` на `succeeded`
- Автоматически создается подписка или покупка курса

### Ручное подтверждение (только для экстренных случаев)
- Если webhook не сработал в течение 1-2 минут, пользователь может вручную подтвердить платеж
- Кнопка "Подтвердить" появляется в профиле для платежей в статусе `pending`
- Это резервный механизм, который НЕ должен использоваться в нормальной ситуации
- Если приходится использовать ручное подтверждение - значит webhook настроен неправильно!
